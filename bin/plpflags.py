...
class Platform(object):
  ...
  def __init__(self, config, flags, apps, build_dir):
    ...
    if plt_name == 'gvsoc' or plt_name == 'vp':
      ...
      if self.config.get('pulp_chip') in ['pulp', 'pulpissimo', 'oprecompkw', 'oprecompkw_sfloat', 'oprecompkw_sa', 'oprecompkw_sfloat_sa', 'multino', 'bigpulp', 'bigpulp-z-7045', 'wolfe']:
        plt_name = 'vp'
...
class Flags_internals(object):
  ...
  def genlink(self, path):
    ...
    for app in self.apps:
      ...
      if linker_path is not None and self.config.get('pulp_chip') in [ 'vivosoc3', 'quentin', 'gap', 'vega', 'wolfe' ]:
